...
/**
 * El cierre diario exige el arqueo resuelto, no solo revisado.
 *
 * El cierre congela el saldo **del libro**, no el contado. Cerrar con una
 * diferencia viva dejaba el día archivado diciendo que había una plata que
 * el conteo no encontró, y limpiarlo después obliga a reabrir el período.
 *
 * El área definió que el día se cierra cuando el arqueo está resuelto: o
 * cuadra, o la diferencia se imputó y el libro ya describe el cajón real.
 * Un arqueo `adjusted` llega con su diferencia explicada en
 * `CASH_DIFFERENCE`, así que lo que se exige es que un `reviewed` cuadre.
 *
 * Las dos versiones de la función van enteras y literales: PHPStan rechaza
 * interpolar dentro de `DB::unprepared()`, y con razón.
 */
            ...
            BEGIN
                ...
                IF NEW.period_type = 'daily' THEN
                    SELECT status,
                           ...
                           END,
                           counted_at
                      INTO estado_arqueo, saldo_arqueo, instante_arqueo
                      FROM cash_counts
                     WHERE cash_box_id = NEW.cash_box_id
                       AND currency = NEW.currency
                       AND counted_on = NEW.period_to
