...
/*
| Publicado para endurecer tres valores de fábrica; el resto es el original
| de barryvdh/laravel-dompdf.
|
| - `chroot`: DomPDF solo lee archivos locales dentro de `public/`, que es
|   donde están el membrete y el sello. De fábrica era la raíz del
|   proyecto, `.env` incluido.
| - `enable_javascript`: apagado. Ningún documento del sistema lo usa.
| - `allowed_protocols`: sin `http://` ni `https://`. `enable_remote` ya
|   estaba apagado; esto lo deja dicho dos veces.
|
| Hoy ninguna plantilla interpola HTML del usuario —todo va con `{{ }}`—,
| así que nada de esto era explotable. Es para que no lo sea el día que
| alguien escriba un `{!! !!}` o un `src` armado con datos.
*/
